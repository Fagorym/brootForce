...
stringToWord8 = foldl(\acc x -> fromIntegral (ord x): acc) [] -- Не реверснутая строка, коды элементов в порядке от последнего к первому
...
firstListAdditional xs = additional ++ (const:xs)
                                    where const = 128::Word8
                                          dif = 55 - length xs
                                          additional = replicate dif 0::[Word8]
